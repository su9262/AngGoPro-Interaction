...
class Eye:
	...
	def __init__(self,x,y,w,h,roundness,pupil_r,pupil_height,eyelid_roundness):
		self.x = x
		self.y = y
		self.width = w
		self.height = h
...
def timaface(inf):
	...
	leftEye = Eye(centerOfEye-distanceBetweenEyes, 515/2, 260, 230, 260/5, 260/4, 0, 0)
	rightEye = Eye(centerOfEye+distanceBetweenEyes, 515/2, 260, 230, 260/5, 260/4, 0, 0)
	running = True
	while running:
	...
	#	pupilSizeDifference = remap(distanceValue, 0, 10, 0, 20)
		pupilSizeDifference = remap(pygame.mouse.get_pos()[1], 0, 515, 0, 20) # for test
		...
		# Draw Pupil
		pygame.draw.circle(screen, (255,255,255), (int(leftEye.x) + shakeVariationX, int(leftEye.y) + shakeVariationY ), int(leftEye.pupil_r)+pupilSizeDifference)
		pygame.draw.circle(screen, (255,255,255), (int(rightEye.x) + shakeVariationX, int(rightEye.y) + shakeVariationY ), int(leftEye.pupil_r)+pupilSizeDifference)
